// Optional page request hooks for diagnostics, and the parser entry point.
//
...
(function () {
    ...
    const { PLATFORM, safeStringify, decodeBatchExecute } = CT;
    ...
    const emitPageReport = (report) => {
        try {
            console.groupCollapsed(`%c[Chat Toolkit] ${report?.title || 'Report'}`, `color:${CT.model?.PROVIDERS?.[PLATFORM]?.accent || '#46689a'};font-weight:bold;`);
    ...
    // ---- parser dispatcher ----------------------------------------------
    // parser-worker.js is loaded as a content script and runs here. Earlier
    // versions tried a dedicated Worker first, but Firefox does not let a
    // content script start a Worker from a moz-extension:// URL: the Worker
    // is created and then fails to load (observed in Firefox Nightly 158 on
    // every supported site), so every export already used this path. The
    // asynchronous wrapper keeps the call sites unchanged.
    const parserCall = async (cmd, args = {}) => {
        if (typeof CT.runParserCommand !== 'function') throw new Error('Parser unavailable');
        return CT.runParserCommand(cmd, args);
    };

    CT.enablePageHooks = () => {
        if (browser.extension?.inIncognitoContext) {
            throw new Error('Page hooks are disabled in private windows');
        }
        pageHooksEnabled = true;
    };
    CT.pageHooksEnabled = pageHooksAllowed;
