...
Keep runtime code readable and local. The extension has no bundler or third-party runtime dependencies. Command labels, provider accents, icons, and palette styles live in `extension-src/lib/ui-model.js`; the page palette and toolbar popup both read them, so change a command there once. Regenerate icons from `assets/design/chat-toolkit-mark.svg` with `python scripts/build-icons.py`. Provider requests must remain on the current provider's HTTPS origin. Diagnostics stay explicit, bounded, and scoped to the initiating tab. Treat all provider content as untrusted text when rendering HTML. Preserve full available conversation history: exports may remove repetition but never shorten content. Diagnostic preview limits must not affect ordinary exports.
